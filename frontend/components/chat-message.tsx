...
export interface Message {
  ...
  suggestions?: string[]
  askFeedback?: boolean
  feedbackLabels?: { yes: string; no: string }  // ADD THIS
}
...
export function ChatMessage({ message, onSuggestionClick, onFeedback }: ChatMessageProps) {
  ...
  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 px-5 py-4 mt-2 bg-gradient-to-r from-slate-50 to-white border border-border/50 rounded-xl">
    <span className="text-sm font-medium text-foreground">
      {message.feedbackLabels ? "What would you like to do?" : "Was your issue resolved?"}
    </span>
    <div className="flex gap-2">
      <Button
        size="sm"
        disabled={feedbackGiven}
        className="h-9 gap-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl shadow-sm shadow-emerald-500/20 transition-all disabled:opacity-50"
        onClick={() => { setFeedbackGiven(true); onFeedback?.(true) }}
      >
        <CheckCircle2 className="h-4 w-4" />
        {message.feedbackLabels?.yes ?? "Yes, resolved"}
      </Button>
      <Button
        variant="outline"
        size="sm"
        disabled={feedbackGiven}
        className="h-9 gap-2 border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 rounded-xl transition-all disabled:opacity-50"
        onClick={() => { setFeedbackGiven(true); onFeedback?.(false) }}
      >
        <XCircle className="h-4 w-4" />
        {message.feedbackLabels?.no ?? "No, need help"}
      </Button>
    </div>
  </div>
)}
